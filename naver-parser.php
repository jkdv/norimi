...
/*
    mysql
 */
if (!($mysqli = new mysqli("localhost", "rssparser", "YndeLMGbvBZGTdsL", "rssparser"))) {
	file_put_contents('naver-paser.txt', 'Fail to make a mysqli instance');
	exit();
}

if (mysqli_connect_errno()) {
	file_put_contents('naver-paser.txt', 'Connect failed: ' . mysqli_connect_error());
	exit();
}

if (!$mysqli->set_charset("UTF8")) {
	file_put_contents('naver-paser.txt', 'Error loading character set utf8: ' . $mysqli->error);
	exit();
}
