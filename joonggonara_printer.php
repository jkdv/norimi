...
define('JONGGO_URL_PRINTER', 	'http://cafe.naver.com/joonggonara.cafe/ArticleList.nhn?search.clubid=10050146&search.menuid=388&search.boardtype=L&search.clubid=10050146');
define('JONGGO_URL_DSLR', 		'http://cafe.naver.com/joonggonara.cafe/ArticleList.nhn?search.clubid=10050146&search.menuid=335&search.boardtype=L&search.clubid=10050146');
define('JONGGO_FIND_ME', '2130');
define('JONGGO_DIR_LOG', './log/');
...
if (!($mysqli = new mysqli(MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD, MYSQL_DATABASE))) {
	printf('Fail to create an instance');
	exit();
}
...
$url 	= JONGGO_URL_PRINTER;
$findme = JONGGO_FIND_ME;
...
foreach ($DOM_Document->find('div[class=article-board m-tcol-c]') as $div_element) {
	foreach ($div_element->find('tr[align=center]') as $tr_element) {
		...
		*/
		...
		/*
			INSERT INTO `rsslist`(`title`, `link`, `pub_date`, `category`, `guid`)
			VALUES (?,?,?,null,?)
		*/
		...
		*/
		if ($stmt->affected_rows > 0 &&
			stripos($title, $findme) !== false) {
			$to      = 'jason.heetae.kim@me.com';
			$subject = $title;
			$message = $link;
			$headers = 'From: norimi@xs.p.ht' . "\r\n" .
			'Reply-To: norimi@xs.p.ht' . "\r\n" .
		    'X-Mailer: PHP/' . phpversion();
			mail($to, $subject, $message, $headers);
		}
	}
	$log = '[' . date($date_format) . '][succeeded][PRINTER  ]' . "\r\n";
	file_put_contents(JONGGO_DIR_LOG . date("Ymd") . '_log.txt', $log, FILE_APPEND);
}
